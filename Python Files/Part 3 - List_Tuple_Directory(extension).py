...
while Proceed=='y':
    ...
    if total_credits!=120:
        ...
        progress_data.append("Progress - %s"%str(tuple)[1:-1])  #converting tuple to string because tuple immutable and cannot be appended to a list
    elif Credit_Pass==100:
        Trailer+='*'
        print('Progress (module trailer)\n')
        tuple=(Credit_Pass,Credit_Defer,Credit_Fail)
        progress_data.append("Progress (module trailer) - %s"%str(tuple)[1:-1])  #https://stackoverflow.com/a/43089203  https://docs.python.org/3/library/stdtypes.html#old-string-formatting
    elif Credit_Fail>=80:
        Exclude+='*'
        print('Exclude\n')
        tuple= (Credit_Pass,Credit_Defer,Credit_Fail)
        progress_data.append("Exclude - %s"%str(tuple)[1:-1])
    else:
        print('Module retriever\n')
        Retriever+='*'
        tuple=(Credit_Pass,Credit_Defer,Credit_Fail)
        progress_data.append("Module retriever - %s"%str(tuple)[1:-1])
    while True:
